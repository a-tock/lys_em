...
class MultiSlice_test(unittest.TestCase):
    ...
    def test_FunctionSpace(self):
        sp = FunctionSpace.fromCrystal(self.Au, 10, 10)
        ...
        sp = FunctionSpace.fromCrystal(self.Au_single, 512 * 2, 512 * 2)
    ...
    def test_Propagation(self):
        ...
        sp = FunctionSpace.fromCrystal(crys, 128, 128)
        b = TEM(60e3)
        V_rs = _Slices(crys, sp, division=10).getPotentialTerms(b)
        pot = _Potentials(V_rs, sp.kvec, crys.unit[2][0], crys.unit[2][1], 1)

        phi = np.zeros((128, 128))
        phi[0][0] = 1

        P_k = sp.getPropagationTerm(b.wavelength, crys.c/10)
...
class CrystalPotential_test(unittest.TestCase):
    ...
    def test_Slices(self):
        ...
        sp = FunctionSpace.fromCrystal(self.Au, 10, 10)
        b = TEM(60e3, 0)
        slices = _Slices(self.Au, sp, division=9)
        n = np.sum([len(s.atoms) for s in slices._slices])
        self.assertEqual(n, len(self.Au.atoms))

        sp = FunctionSpace.fromCrystal(self.TaTe2, 10, 10)
        slices = _Slices(self.TaTe2, sp, division=9)
        n = np.sum([len(s.atoms) for s in slices._slices])
        ...
        sp = FunctionSpace.fromCrystal(self.Au, 10, 10)
        slices = _Slices(self.Au, sp, division=1)
